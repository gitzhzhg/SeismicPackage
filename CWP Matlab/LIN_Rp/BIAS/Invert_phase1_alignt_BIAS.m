%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Modification of Invert_alignt_phase1.m to evaluate
% the bias => this inversion does not see the
% terms which are suppose to be zero
%
% Inversion of Rpp and Rps from data file - PHASE1. 
% This code assumes kappa=0, i.e., alignment !!!
% In PHASE1, parameters of the surfaces Rpp(inc_angle,azim)  
...
n_inv_p=4;      % number of inverting variables in PP (9)
n_inv_s=8;     % number of inverting variables in PS (18)
std_p=0.0;        % standard deviation for PP (in %)
std_s=0.0;        % standard deviation for PS (in %)
...
%Rpp=Rpp_ORT;
%Rsvsh=Rsvsh_ORT;
%load Rpp_bias.out   % Rpp data
%load Rsvsh_bias.out % Rps data
%Rpp=Rpp_bias;
%Rsvsh=Rsvsh_bias;
%Pdata=Rpp(:,3);
%Sdata=Rsvsh(:,3);
...
AS(:,3)=[sin(S_inc).^3];
AS(:,4)=[(sin(S_az).^2).*(sin(S_inc).^3)];
AS(:,5)=[sin(S_inc).^5];
AS(:,6)=[(sin(S_az).^2).*(sin(S_inc).^5)];
AS(:,7)=[sin(S_inc).^7];
AS(:,8)=[(sin(S_az).^2).*(sin(S_inc).^7)];
...
%     DATAS=dataS';                          % if you read in data from a file
 ...
 %  fprintf(foutP, ['%10.4f'], DATAP);
 %  foutS = fopen('dataS.out', 'w');
 %  fprintf(foutS, ['%10.4f '], DATAS);
Pdata=DATAP;
Sdata=DATAS;
...
hold;
T1=plot(S_inc_deg,S_az_deg,'ro','MarkerSize',8);
%set(T1,'Color',[0 0 0]);
...
if U == 0
  ...
  fid=fopen('Result_phase1_bias.out','r+');
  if fid==-1
