!!------------------------ dist_wrap.f90 --------------------------!!
!!------------------------ dist_wrap.f90 --------------------------!!
!!------------------------ dist_wrap.f90 --------------------------!!

!!!!!!!!!! DO NOT EDIT THIS FILE - it is machine generated !!!!!!!!!!

!!------------------------------ module -----------------------------------!!
!!------------------------------ module -----------------------------------!!
!!------------------------------ module -----------------------------------!!

      module dist_wrap_module
      use dist_module
      implicit none
      public

      type :: dist_wrap_struct
        type(dist_struct),pointer :: obj
      end type dist_wrap_struct

      end module dist_wrap_module

!!---------------------------- create ------------------------------------!!
!!---------------------------- create ------------------------------------!!
!!---------------------------- create ------------------------------------!!

      subroutine dist_wrap_create (fpoint)  
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(out)   :: fpoint
      type(dist_struct)     ,pointer       :: obj           ! local

      nullify (obj)               ! needed for intel compiler.
      call dist_create (obj)
      fpoint%obj => obj

      end subroutine dist_wrap_create

!!---------------------------- delete ------------------------------------!!
!!---------------------------- delete ------------------------------------!!
!!---------------------------- delete ------------------------------------!!

      subroutine dist_wrap_delete (fpoint)   
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(inout) :: fpoint
      type(dist_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call dist_delete (obj)
      fpoint%obj => obj

      end subroutine dist_wrap_delete

!!---------------------------- update ------------------------------------!!
!!---------------------------- update ------------------------------------!!
!!---------------------------- update ------------------------------------!!

      subroutine dist_wrap_update (fpoint)     
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(inout) :: fpoint
      type(dist_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call dist_update (obj)

      end subroutine dist_wrap_update

!!---------------------------- wrapup ------------------------------------!!
!!---------------------------- wrapup ------------------------------------!!
!!---------------------------- wrapup ------------------------------------!!

      subroutine dist_wrap_wrapup (fpoint)      
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(inout) :: fpoint
      type(dist_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call dist_wrapup (obj)

      end subroutine dist_wrap_wrapup

!!---------------------------- oneset ------------------------------------!!
!!---------------------------- oneset ------------------------------------!!
!!---------------------------- oneset ------------------------------------!!

      subroutine dist_wrap_oneset (fpoint,ntr,  &
                                   hd,tr,lenhd,lentr,num)
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(inout) :: fpoint
      integer               ,intent(in)    :: lenhd,lentr,num  ! argument
      integer               ,intent(inout) :: ntr              ! argument
      double precision      ,intent(inout) :: hd(lenhd,num)    ! argument
      real                  ,intent(inout) :: tr(lentr,num)    ! argument
      type(dist_struct)     ,pointer       :: obj              ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call dist (obj,ntr,hd,tr)                ! might be commented out.

      end subroutine dist_wrap_oneset

!!---------------------------- twosets ------------------------------------!!
!!---------------------------- twosets ------------------------------------!!
!!---------------------------- twosets ------------------------------------!!

      subroutine dist_wrap_twosets (fpoint,ntr,                 &
                                    hd1,tr1,lenhd1,lentr1,num1, &
                                    hd2,tr2,lenhd2,lentr2,num2)
      use dist_wrap_module
      implicit none
      type(dist_wrap_struct),intent(inout) :: fpoint
      integer               ,intent(in)    :: lenhd1,lentr1,num1 ! argument
      integer               ,intent(in)    :: lenhd2,lentr2,num2 ! argument
      integer               ,intent(inout) :: ntr                ! argument
      double precision      ,intent(inout) :: hd1(lenhd1,num1)   ! argument
      real                  ,intent(inout) :: tr1(lentr1,num1)   ! argument
      double precision      ,intent(inout) :: hd2(lenhd2,num2)   ! argument
      real                  ,intent(inout) :: tr2(lentr2,num2)   ! argument
      type(dist_struct)     ,pointer       :: obj                ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
!!!!  call dist (obj,ntr,hd1,tr1,hd2,tr2)      ! might be commented out.

      end subroutine dist_wrap_twosets

!!----------------------------- end ---------------------------------------!!
!!----------------------------- end ---------------------------------------!!
!!----------------------------- end ---------------------------------------!!
