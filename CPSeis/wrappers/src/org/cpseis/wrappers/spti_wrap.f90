!!------------------------ spti_wrap.f90 --------------------------!!
!!------------------------ spti_wrap.f90 --------------------------!!
!!------------------------ spti_wrap.f90 --------------------------!!

!!!!!!!!!! DO NOT EDIT THIS FILE - it is machine generated !!!!!!!!!!

!!------------------------------ module -----------------------------------!!
!!------------------------------ module -----------------------------------!!
!!------------------------------ module -----------------------------------!!

      module spti_wrap_module
      use spti_module
      implicit none
      public

      type :: spti_wrap_struct
        type(spti_struct),pointer :: obj
      end type spti_wrap_struct

      end module spti_wrap_module

!!---------------------------- create ------------------------------------!!
!!---------------------------- create ------------------------------------!!
!!---------------------------- create ------------------------------------!!

      subroutine spti_wrap_create (fpoint)  
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(out)   :: fpoint
      type(spti_struct)     ,pointer       :: obj           ! local

      nullify (obj)               ! needed for intel compiler.
      call spti_create (obj)
      fpoint%obj => obj

      end subroutine spti_wrap_create

!!---------------------------- delete ------------------------------------!!
!!---------------------------- delete ------------------------------------!!
!!---------------------------- delete ------------------------------------!!

      subroutine spti_wrap_delete (fpoint)   
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(inout) :: fpoint
      type(spti_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call spti_delete (obj)
      fpoint%obj => obj

      end subroutine spti_wrap_delete

!!---------------------------- update ------------------------------------!!
!!---------------------------- update ------------------------------------!!
!!---------------------------- update ------------------------------------!!

      subroutine spti_wrap_update (fpoint)     
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(inout) :: fpoint
      type(spti_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call spti_update (obj)

      end subroutine spti_wrap_update

!!---------------------------- wrapup ------------------------------------!!
!!---------------------------- wrapup ------------------------------------!!
!!---------------------------- wrapup ------------------------------------!!

      subroutine spti_wrap_wrapup (fpoint)      
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(inout) :: fpoint
      type(spti_struct)     ,pointer       :: obj           ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call spti_wrapup (obj)

      end subroutine spti_wrap_wrapup

!!---------------------------- oneset ------------------------------------!!
!!---------------------------- oneset ------------------------------------!!
!!---------------------------- oneset ------------------------------------!!

      subroutine spti_wrap_oneset (fpoint,ntr,  &
                                   hd,tr,lenhd,lentr,num)
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(inout) :: fpoint
      integer               ,intent(in)    :: lenhd,lentr,num  ! argument
      integer               ,intent(inout) :: ntr              ! argument
      double precision      ,intent(inout) :: hd(lenhd,num)    ! argument
      real                  ,intent(inout) :: tr(lentr,num)    ! argument
      type(spti_struct)     ,pointer       :: obj              ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
!!!!  call spti (obj,ntr,hd,tr)                ! might be commented out.

      end subroutine spti_wrap_oneset

!!---------------------------- twosets ------------------------------------!!
!!---------------------------- twosets ------------------------------------!!
!!---------------------------- twosets ------------------------------------!!

      subroutine spti_wrap_twosets (fpoint,ntr,                 &
                                    hd1,tr1,lenhd1,lentr1,num1, &
                                    hd2,tr2,lenhd2,lentr2,num2)
      use spti_wrap_module
      implicit none
      type(spti_wrap_struct),intent(inout) :: fpoint
      integer               ,intent(in)    :: lenhd1,lentr1,num1 ! argument
      integer               ,intent(in)    :: lenhd2,lentr2,num2 ! argument
      integer               ,intent(inout) :: ntr                ! argument
      double precision      ,intent(inout) :: hd1(lenhd1,num1)   ! argument
      real                  ,intent(inout) :: tr1(lentr1,num1)   ! argument
      double precision      ,intent(inout) :: hd2(lenhd2,num2)   ! argument
      real                  ,intent(inout) :: tr2(lentr2,num2)   ! argument
      type(spti_struct)     ,pointer       :: obj                ! local

      obj => fpoint%obj
      if (.not. associated(obj)) return
      call spti (obj,ntr,hd1,tr1,hd2,tr2)      ! might be commented out.

      end subroutine spti_wrap_twosets

!!----------------------------- end ---------------------------------------!!
!!----------------------------- end ---------------------------------------!!
!!----------------------------- end ---------------------------------------!!
