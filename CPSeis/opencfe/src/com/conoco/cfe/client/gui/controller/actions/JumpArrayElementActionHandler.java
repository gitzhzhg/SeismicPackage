// JumpArrayElementActionHandler.java
...
/**
 * Action handler for the "JumpArrayElement" action.
 */
public class JumpArrayElementActionHandler
   ...
   */
  public JumpArrayElementActionHandler(AppGUIController.ActionHandlerHelper helper) {
    super(helper);  
  }
  ...
  public void performAction(Node n) {
    ...
    int elementNo = XMLHelper.getIntAttributeValue(n, "value")-1;
    
    GUIControl control = getHelper().getControl(pid, keyword);
    
    if (control != null) {
      com.conoco.cfe.utils.EventQueue.invokeLater(
        new RequestFocusRunnable(pid, control, elementNo));
    }
    else {
      //Console.logMessage("JumpArrayElementActionHandler: Couldn't find component " + keyword);
    }
  }        
