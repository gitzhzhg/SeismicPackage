...
typedef struct memBitBuffStruct{
	memBUFF *buff;
	unsigned char mask;
	unsigned char rack;
} memBITBUFF;
...
/* bitInitbuff(bitbuff, buff)
   ...
   bitWindbuff(bitbuff, flag)
	...
	_flag_ = 0 if successful, _MEM_EOB_ otherwise
   ...
   bitInputbit(bitbuff, flag)
	...
	_bitbuff_, _flag_ = 0 or 1 if successful, _MEM_EOB_ otherwise
...
#define bitInitbuff(bitbuff, buff1){\
	(bitbuff) = (memBITBUFF *) malloc(sizeof(memBITBUFF));\
	(bitbuff)->buff = (buff1);\
	(bitbuff)->mask = 0x80;\
	(bitbuff)->rack = 0x0;\
}
...
/* huffman.c:135: warning: suggest explicit braces to avoid ambiguous `else' */
#define bitWindbuff(bitbuff, flag){\
	(flag) = 0;\
	if((bitbuff)->mask != 0x80){\
	    if(buffPutc((bitbuff)->buff, (bitbuff)->rack) == MEM_EOB)\
		(flag) = MEM_EOB;\
	    else (flag) = 1;}\
}
...
#define bitInputbit(bitbuff, flag){\
	...
	(flag) = 0;\
	if((bitbuff)->mask == 0x80)\
	    if(buffGetc((bitbuff)->buff, (bitbuff)->rack) == MEM_EOB)\
		(flag) = MEM_EOB;\
	if((flag) != MEM_EOB){\
