...
/* SUPSWIGP: $Revision: 1.37 $ ; $Date: 2011/11/12 00:46:15 $		*/
...
{
	char *plotcmd;		/* build pswigp command for popen	*/
	...
	if (!have_ntr || irregular ) { /* count traces */
		if (verbose) {
			if( irregular ) {
				warn("trace spacing from key %s",key);
	...
	if (irregular ) {
		...
		sprintf(plotcmd,
			"%spswigp n1=%d d1=%f f1=%f %s style=%s fill=%d", bindir,
			   nt, d1, f1, x2string, style, fill);
		free(x2string);
	} else {
		...
		sprintf(plotcmd,
		 "%spswigp n1=%d n2=%d d1=%f d2=%f f1=%f f2=%f style=%s fill=%d", bindir,
			   nt, ntr, d1, d2, f1, f2, style, fill);
		}
	...
	} else { /* just pump out traces and let pswigp do the work */
		do {
