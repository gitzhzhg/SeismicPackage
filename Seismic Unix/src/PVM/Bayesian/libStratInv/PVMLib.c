...
/* Clean PSU log directory */
  ...
  sprintf(command, "rm -rf %s/INV*\0", hd);
  system(command);
...
/* Open the log file */
	...
	/* creating log files */
	sprintf(log_file, "%s/INV%s%d\0", hd, myhost, node -> NodeTid);
...
char* RecvStr(int tid, int MsgType)
{
  ...
  pvm_bufinfo(BufId, &NBytes, &MsgTag, &MasterTid);
  info = pvm_upkstr(tmp_char);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvStr : Message type %d\n", MsgType);
  }
...
int RecvInt(int *IntBuf, int n, int tid, int MsgType)
{
  ...
  info = pvm_upkint(IntBuf, n, 1);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvInt: Message type %d\n", MsgType);
  }
  return(SenderTid);
}
...
int RecvFloat(float *FloatBuf, int n, int tid, int MsgType)
{
  ...
  BufId = pvm_recv(-1, MsgType);
  info  = pvm_bufinfo( BufId, &NBytes, &MsgTag, &SenderTid);
  info = pvm_upkfloat(FloatBuf, n, 1);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvFloat : Message type %d\n", MsgType);
  }
...
int RecvCplx(complex *CplxBuf, int n, int tid, int MsgType)
{
  ...
  BufId = pvm_recv(-1, MsgType);
  info  = pvm_bufinfo(BufId, &NBytes, &MsgTag, &SenderTid);
  info = pvm_upkbyte((char *) CplxBuf, n*sizeof(complex), 1);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvCplx : Message type %d\n", MsgType);
  }
  return(SenderTid);
}
...
int RecvFI(float *FloatBuf, int nf, int *IntBuf, int ni, int tid, int MsgType)
{
  ...
  info = pvm_upkbyte((char *)FloatBuf, nf*sizeof(float), 1);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvFI : Message type %d\n", MsgType);
  }
  return(SenderTid);
}
...
int RecvINFO(INFO *data, int n, int tid, int MsgType)
{
  ...
  BufId = pvm_recv(-1, MsgType);
  info  = pvm_bufinfo(BufId, &NBytes, &MsgTag, &SenderTid);
  info = pvm_upkbyte((char *) data, n*sizeof(INFO), 1);
  if (info < 0)
  {
     fprintf(stderr, "Problems with RecvINFO : Message type %d\n", MsgType);
  }
  return(SenderTid);
}
...
*****************************************************************************/
{
   ...
   /* Save return for next entry   */
   time_t t1, t2;
   /* Resp. last and current times */
   if (firsttime) 
   {
      firsttime = 0;
      lasttime = time(&t1);
