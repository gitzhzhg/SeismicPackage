...
void
dgeco (double **a, int n, int *ipvt, double *rcond, double *z)
/*****************************************************************************
...
{
	int info,j,k,kp1,l;
	double ek,t,wk,wkm,anorm,s,sm,ynorm;
	...
	/* solve L'y = w */
	for (k=n-1; k>=0; k--) {
		if (k<n-1) z[k] += ddot(n-k-1,&a[k][k+1],1,&z[k+1],1);
		if (ABS(z[k])>1.0) {
			s = 1.0/ABS(z[k]);
			dscal(n,s,z,1);
		}
	...
	for (k=0; k<n; k++) {
		...
		z[l] = z[k];
		z[k] = t;
		if (k<n-1) daxpy(n-k-1,t,&a[k][k+1],1,&z[k+1],1);
		if (ABS(z[k])>1.0) {
			s = 1.0/ABS(z[k]);
			dscal(n,s,z,1);
			ynorm *= s;
...
*****************************************************************************/
{
	...
	/* else, if solving A'x = b */
		...
		/* now solve L'x = y */
		for (k=n-2; k>=0; k--) {
			b[k] += ddot(n-k-1,&a[k][k+1],1,&b[k+1],1);
			l = ipvt[k];
			if (l!=k) {
				t = b[l];
				b[l] = b[k];
