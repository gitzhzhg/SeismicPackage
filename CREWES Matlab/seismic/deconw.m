...
% [trout,d]=deconw(trin,trdsign,n,stab)
% [trout,d]=deconw(trin,trdsign,n)
%
...
% deconvolve trin
  trout=convm(trin,x);
%   w=ifft(1 ./fft(x)); %get wavelet
%   sw=convm(trout,w);%estimated trace
%   [diff,scalar]=lsqsubtract(trin,sw);
%   trout=scalar*trout;
