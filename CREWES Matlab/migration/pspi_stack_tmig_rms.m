function [zosmig,exzos]=pspi_stack_tmig_rms(zos,t,x,vrms,xv,tau,frange,taucheck)
% PSPI_STACK_TMIG: zos (zero offset section) TIME migration by the PSPI algorithm
%
% [zosmig,exzos]=pspi_stack_tmig_rms(zos,t,x,vrms,xv,tau,frange,zcheck)
%
% PSPI_STACK_TMIG_RMS performs 2D time migration for PP events on a stacked
...
% vrms...velocity model in time. Velocities should be RMS not interval. 
...
% NOTE: size(vrms) must equal [length(tau), length(xv)]
...
if(Nxv==1 || Ntv==1)%allow for either as a row vector may be input
    vrms=vrms(:);%force a column
    vrms=vrms*ones(1,nx);%replicate across x
    [Ntv,Nxv]=size(vrms);
    xv=x;
end
...
if(nx<nx2)
    ...
    vrms=[vrms vrms(:,end)*ones(1,nx2-nx)];%extend vel with last trace
end
x2=(0:nx2-1)*dx;
...
vel_blocked=Bagaini(length(x)-1,10,vrms);
...
%     end
	...
	zosmfx(indf,:)=pspi_ipsf(zosfk(indf,:),f(indf),dx,vrms(jvel,:),vel_blocked(jvel,:),dtaustep.*mean(vrms(j,:)));
